...
where
    W: ChunkStorage<W, T>,
    T: BlockData, {
    /// Marks the chunk at the given chunk coordinates as loading.
    ///
    /// The chunk generated by this function is not a full chunk and should not
    /// be readable until [`init_chunk`] is called on the chunk.
    ///
    /// This function will return an error if called upon a chunk that already
    /// exists or is currently being loaded.
    ...
    ///
    /// This function does not require the chunk to be prepared with
    /// [`prepare_chunk`] and can safely be called on either unloaded chunks
    /// or loading chunks.
    ///
...
/// Defines the current load state of a chunk.
...
pub enum ChunkLoadState {
    ...
    /// Indicates that the chunk is actively being loading in an async thread
    /// and should not yet be used.
    Loading,

    /// Indicates that the chunk is fully loaded and ready to be used.
    Loaded,
}
